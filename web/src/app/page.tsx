...
import { Send, Menu, Sparkles, MessageSquare, Terminal, Cpu } from "lucide-react";
...
function ChatContent() {
  ...
  const loadConversation = useCallback(async (convId: string) => {
    ...
    try {
      ...
      setMessages(msgs);
      chatHistory.current = msgs.map((m) => ({ role: m.role, content: m.content }));
    } catch {
      setMessages([]);
      chatHistory.current = [];
    }
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--background)]">
      ...
      <header className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border)] bg-[var(--background)]/80 backdrop-blur-md sticky top-0 z-10">
        <button onClick={toggleSidebar} className="text-[var(--muted)] hover:text-white p-1.5 hover:bg-[var(--card)] rounded-lg transition-colors">
          <Menu size={18} />
        </button>
        ...
        <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-8">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[var(--muted)] max-w-sm">
                  我是您的 CS 专属导师。您可以问我关于算法、操作系统、计算机组成原理等相关问题。
                </p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-md px-4">
                {SUGGESTIONS.map((s) => (
                  ...
                  >
                    <span className="text-[var(--muted)] group-hover:text-[var(--accent)]">{s.icon}</span>
          ...
          {messages.map((m) => (
            ...
            >
              ...
              <span className="mt-1 text-[10px] text-[var(--muted)] opacity-0 group-hover:opacity-100 transition-opacity px-1">
          ...
          {thinkingText && (
            <div className="flex items-start">
              <div className="flex items-center gap-3 rounded-2xl px-5 py-3 bg-[var(--card)] border border-[var(--border)]">
                ...
                <span className="text-sm text-[var(--muted)] thinking-text" key={thinkingText}>{thinkingText}</span>
              </div>
            </div>
          )}
        </div>
